...
public class Computer implements ElectronicDevice, Serializable {
  ...
  public Computer(int size) {
    this.components = new String[size];
    this.model = "Computer::Default";
    this.price = 0;
  }
  ...
  public String getElement(int index) {
    if (index < 0 || index >= components.length) {
      throw new IndexOutOfBoundsException("Индекс не может быть отрицательным и не может превышать " +
              "значения - " + (components.length - 1));
    }
  ...
  public void writeVal(String val, int index) {
    if (index < components.length) {
      ...
      components[components.length - 1] = val;
    }
    System.out.println("Computer Write: " + val + " to position " + index);
  }

  @Override
  public String readVal(int index) {
    String comp = components[index];
    System.out.println("Computer Read: " + comp + " from position " + index);
    return comp;
  }
  ...
  public void fillItemsWithDefaultValues(int count) {
    for (int i = 0; i < count; i++) {
      components[i] = "Computer::DefaultComponent#" + i;
    }
    System.out.println("Computer filled with default values.");
  }

  @Override
  public Iterator<String> iterator() { return new ElectronicDeviceIterator(this); }
