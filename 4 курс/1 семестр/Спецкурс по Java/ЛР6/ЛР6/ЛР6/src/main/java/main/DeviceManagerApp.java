...
public class DeviceManagerApp {
  ...
  public void start() {
    ...
    while (running) {
      ...
      System.out.println("16. Проверить итераторы (while)");
      System.out.println("17. Проверить итераторы (for-each)");
      System.out.println("18. Установить фабрику для компьютеров");
      System.out.println("19. Установить фабрику для смартфонов");
      System.out.println("20. Создать устройство через фабрику");
      System.out.println("21. Создать неизменяемую коллекцию");
      ...
      switch (choice) {
        ...
        case 16:
          ...
          break;
        case 17:
          testIteratorsForEach();
          break;
        case 18:
          setComputerFactory();
          break;
        case 19:
          setSmartphoneFactory();
          break;
        case 20:
          createDeviceThroughFactory();
          break;
        case 21:
  ...
  private void createUnmodifiableCollection() {
    ElectronicDevice device = DeviceHandler.createInstance(3);
    ElectronicDevice unmodifiableCollection = DeviceHandler.unmodifiableDevice(device);
    try {
      unmodifiableCollection.setModel("NewName");
    } catch (UnsupportedOperationException e) {
      System.out.println("Перехваченное ожидаемое исключение: " + e.getMessage());
    }
    System.out.println("Неизменяемая коллекция создана.");
  }
  ...
  private void writeDevicesToByteStream() {
    try (FileOutputStream fos = new FileOutputStream("devices.dat")) {
      for (ElectronicDevice device : devices) {
        DeviceHandler.outputElectronicDevice(device, fos);
      }
  ...
  private void readDevicesFromByteStream() {
    try (FileInputStream fis = new FileInputStream("devices.dat")) {
      ...
      while ((device = DeviceHandler.inputElectronicDevice(fis)) != null) {
        devices.add(device);
      }
  ...
  private void writeDevicesToCharStream() {
    try (FileWriter writer = new FileWriter("devices.txt")) {
      for (ElectronicDevice device : devices) {
        DeviceHandler.writeElectronicDevice(device, writer);
      }
  ...
  private void readDevicesFromCharStream() {
    try (FileReader reader = new FileReader("devices.txt")) {
      ArrayList<ElectronicDevice> devices = DeviceHandler.readElectronicDevice(reader);
  ...
  private void serializeDevices() {
    try (FileOutputStream fos = new FileOutputStream("devices.ser")) {
      for (ElectronicDevice device : devices) {
        DeviceHandler.serializeElectronicDevice(device, fos);
      }
  ...
  private void deserializeDevices() {
    try (FileInputStream fis = new FileInputStream("devices.ser")) {
      while (fis.available() > 0) {
        ElectronicDevice device = DeviceHandler.deserializeElectronicDevice(fis);
        devices.add(device);
      }
  ...
  private void formatReadDevices() {
    try (Scanner sc = new Scanner(Paths.get("devices.format.txt"), "UTF-8")) {
      ElectronicDevice device;
      while ((device = DeviceHandler.readFormat(sc)) != null) {
        devices.add(device);
      }
      System.out.println("Устройства успешно прочитаны из байтового потока.");
    } catch (Exception e) {
  ...
  private void formatWriteDevices() {
    try (FileWriter writer = new FileWriter("devices.format.txt")) {
      for (ElectronicDevice device : devices) {
        DeviceHandler.writeFormat(device, writer);
      }
      System.out.println("Устройства успешно записаны в текстовый поток.");
    } catch (IOException e) {
      System.out.println("Ошибка при записи устройств: " + e.getMessage());
  ...
  public static void testThreadSafeDevice() {
    ElectronicDevice device = new Computer(100);
    device.fillItemsWithDefaultValues(100);
    device = DeviceHandler.getThreadSafeDevice(device);
  ...
  private void testIteratorsWhile() {
    for (ElectronicDevice device : devices) {
  ...
  private void testIteratorsForEach() {
    for (ElectronicDevice device : devices) {
      System.out.println(device.getModel());
      for (String component : device) {
        System.out.println(component);
      }
    }
  }

  private void setComputerFactory() {
    DeviceHandler.setFactory(new ComputerFactory());
    System.out.println("Фабрика для компьютеров установлена.");
  }

  private void setSmartphoneFactory() {
    DeviceHandler.setFactory(new SmartphoneFactory());
    System.out.println("Фабрика для смартфонов установлена.");
  }

  private void createDeviceThroughFactory() {
    ElectronicDevice device = DeviceHandler.createInstance(3);
    System.out.println("Устройство создано через фабрику: " + device.toString());
    devices.add(device);
  }
}